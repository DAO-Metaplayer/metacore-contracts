...
const DOMAIN = ethers.utils.arrayify(ethers.utils.solidityKeccak256(["string"], ["CUSTOM_Metachain_MANAGER"]));
...
async function generateRegistrationSignature(signerAddress: string, contractAddress: string) {
  ...
  const { signature } = mcl.signMetachainMessage(DOMAIN, CHAIN_ID, signerAddress, contractAddress, secret);
