...
async function generateMsg() {
  await mcl.init();
  const message = ethers.utils.hexlify(ethers.utils.toUtf8Bytes("Metachain-V1-validator"));
  ...
  const messagePointForInit = mcl.g1ToHex(
    mcl.hashToPoint(ethers.utils.hexlify(ethers.utils.toUtf8Bytes("Metachain-V1-validator")), DOMAIN)
  );
